...
uses
  ...
  custhttpapp, fpwebfile,base64,openssl,sslsockets,opensslsockets, fpjson, jsonparser,sql_worker,strutils;
 ...
 { RESTful API application }
  ...
  public
   ...
   // procedure Tasks(req: Trequest; res: TResponse);
    ...
    procedure get_decoy_output(req : Trequest; res: TResponse);


    procedure agent_heart_beat(req : Trequest; res: TResponse);
    procedure agent_tasks(req: Trequest; res: TResponse);
    procedure tasks_update_status(req: Trequest; res: TResponse);

    procedure decoy_tasks_result(req: Trequest; res: TResponse);
...
function get_value(lookup:string):string;
var
  list : Tstringlist;
begin
       try
       ...
       list.loadfromfile(getcurrentdir+'/config.c2c');
...
{still in progress}
  ...
  //agent_heart_beat_v := get_value;



  HTTPRouter.RegisterRoute('/agent/heart_beat',@agent_heart_beat);

  HTTPRouter.RegisterRoute('/tasks/view/',@agent_tasks);

 HTTPRouter.RegisterRoute('/tasks/update',@tasks_update_status);

 HTTPRouter.RegisterRoute('/tasks/results',@decoy_tasks_result);  // this is to insert decoy result
...
procedure TPasserver.decoy_tasks_result(req: Trequest; res: TResponse);
var
  JSON : TJSONObject;
  task_id,task_body,UUID:string;
  HTTPCODE : integer;
begin
TRY
...
with DB do begin
 insert_task_result(UUID,task_id,task_body);
end;

json.Add('task_id',task_id);
httpCode := 200;
...
procedure TPasserver.tasks_update_status(req: Trequest; res: TResponse);
var
  JSON : TJSONObject;
  httpcode : integer;
  task_id,task_status,UUID : string;
begin
 try
JSON := Tjsonobject.Create;

task_id := req.ContentFields.Values['task_id'];
task_status := req.ContentFields.Values['task_status'];
 UUID := req.ContentFields.Values['uuid'];

with DB do begin

update_task(uuid,task_id,task_status);

end;
json.Add('task_id',task_id);
json.Add('task_status',task_status);
httpCode := 200;
...
procedure TPasserver.agent_tasks(req : Trequest; res: TResponse);
var
JSON : TJSONObject;
httpcode : integer;
profile_UUID : string;
 task_data,task_status,task_id: string;
begin

/// this end-point is only activated for agent side.
try
JSON := Tjsonobject.Create;


profile_UUID := req.QueryFields.Values['profile_UUID'];


with DB do begin

// check if the decoy is here, then if yes get it own task
get_task(profile_UUID,task_data,task_status,task_id);

end;
json.Add('task_body',task_data);
json.Add('task_status',task_status);
json.Add('task_id',task_id);
httpCode := 200;
...
procedure TPasserver.get_decoy_output(req : Trequest; res: TResponse); // this is for operator side
var
  jObject : TJSONobject;
  UUID,task_id:string;
  task_output: string;
...
with DB do begin
 get_decoy_result(UUID,task_id,task_output);

end;

jObject.Add('UUID',UUID);
jObject.Add('task_id',task_id);
Jobject.Add('task_output',task_output);
 httpCode := 200;
jsonresponse(res,Jobject,httpcode);
...
procedure TPasserver.add_task(req: Trequest; res: TResponse);
 var
  jObject : TJSONobject;
  UUID,Res_str,task_name,task_data,task_id,validation:string;
  httpcode:integer;
begin
...
add_task(UUID,task_name,task_data,task_id);
...
begin
   ...
   _Parent.UseSSL:=true;
   _Parent.CertificateData.HostName := 'fedora';
 ...
 HTTPRouter.RegisterRoute('/decoys/list',@decoys_list);
  ...
  HTTPRouter.RegisterRoute('/decoy/results',@get_decoy_output);
 ...
 // HTTPRouter.RegisterRoute('/tasks/decoy/', @decoy);
  ...
  _Parent.HostName:='192.168.33.136';
  _Parent.UseSSL:=true;
